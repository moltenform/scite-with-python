    ...
    <menubar name="MenuBar">
        <menu name="ToolsMenu" action="Tools">
            <placeholder name="ToolsOps_2">
                <menuitem name="Regex Search" action="RegexSearch"/>
            </placeholder>
        </menu>
    </menubar>
</ui>
...
class RegexSearchInstance:
    ...
    def create_menu_item(self):
        action = gtk.Action("RegexSearch", _("Regex Search"), _("Search using regular expressions"), None)
    ...
    def search_document(self, start_iter = None, wrapped_around = False, button = 'search'):
        ...
        regex = re.compile(unicode(self._search_text_box.get_text(), "utf-8"))
    ...
    def inform_no_match(self):
        dlg = gtk.MessageDialog(self._window,
                                ...
                                gtk.MESSAGE_INFO,
                                gtk.BUTTONS_CLOSE,
                                _(u"The regular expression \"%s\" hasn't any match." % self._search_text_box.get_text()))
        dlg.run()
        dlg.hide()
    ...
    def show_search_result(self, result, document, start_iter,button):
        ...
        if (button == 'replace'):
            ...
            self.replace_text(document,replace_text)

        view = self._window.get_active_view()
        view.scroll_to_cursor()

    def replace_text(self,document,replace_text):
